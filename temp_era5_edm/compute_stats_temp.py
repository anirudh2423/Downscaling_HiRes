"""
Compute normalization statistics for ERA5-conditioned AORC temperature training.

Outputs:
    norm_stats_temp_era5.npz

The script samples timesteps instead of scanning the full 2020 AORC grid.
"""
...
def compute_era5_t2m_stats():
    print("Computing ERA5 2m_temperature stats...", flush=True)
    rng = np.random.default_rng(CFG.seed)
    ds = xr.open_zarr(CFG.era5_zarr, consolidated=None)
    times = ds.time.values.astype("datetime64[s]")
    mask = (times >= np.datetime64("2020-01-01T00:00:00")) & (
        times <= np.datetime64("2020-12-31T23:00:00")
    )
    idxs = np.flatnonzero(mask)
    sample = rng.choice(idxs, size=min(N_ERA5_SAMPLE, len(idxs)), replace=False)

    count = 0
    sum_x = 0.0
    sum_x2 = 0.0
    for t in sample:
        slab = ds["2m_temperature"].isel(time=int(t)).values
        valid = slab[~np.isnan(slab)].astype(np.float64)
        count += valid.size
        sum_x += valid.sum(dtype=np.float64)
        sum_x2 += np.square(valid, dtype=np.float64).sum(dtype=np.float64)
    ...
    var = sum_x2 / count - mean**2
    std = np.sqrt(max(var, 0.0))
    print(f"  ERA5 t2m mean={mean:.6f} std={std:.6f}", flush=True)
    return float(mean), float(std)


def compute_topo_stats():
    print("Computing topo stats on AORC grid...", flush=True)
    ref_ds = xr.open_zarr(CFG.aorc_zarr[0], consolidated=False, drop_variables=CFG.target_vars)
...
def main():
    ...
    era5_t2m_mean, era5_t2m_std = compute_era5_t2m_stats()
    ...
    np.savez(
        ...
        temp_mean=temp_mean,
        temp_std=temp_std,
        era5_t2m_mean=era5_t2m_mean,
        era5_t2m_std=era5_t2m_std,
        topo_mean=topo_mean,
        topo_std=topo_std,
    )
