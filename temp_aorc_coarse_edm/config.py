...
class Config:
    ...
    stats_file: str = "/Datastorage/scdlds_anirudhavireddy/Temp_AORC_Coarse/norm_stats_temp_coarse.npz"
    output_dir: str = "/Datastorage/scdlds_anirudhavireddy/Temp_AORC_Coarse/runs/temp_aorc_coarse_edm"
    plots_dir: str = "/Datastorage/scdlds_anirudhavireddy/Temp_AORC_Coarse/Plots"

    target_vars: list = field(default_factory=lambda: ["TMP_2maboveground"])
    target_channels: int = 1
    patch_size: int = 256
    coarse_size: int = 32
    cond_channels: int = 4       # topo, SVF, CSZA, coarse AORC temperature
    input_channels: int = 5      # 1 noisy temperature channel + cond_channels
    val_fraction: float = 0.1
    nan_threshold: float = 0.6
    index_stride: int = 128

    # Normalisation stats; compute_stats_temp.py writes these before training.
